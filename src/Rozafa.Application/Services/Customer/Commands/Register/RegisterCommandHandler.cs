...
public class RegisterCommandHandler : IRequestHandler<RegisterCommand, ErrorOr<string>>
{
    ...
    {
        if (request.User.LastName == "asd")
        {
            return Errors.Customer.EmptyLastName;
        }

        return await _customer.RegisterCustomer(new Domain.Models.Customer.RegisterCustomer(request.User.FirstName, request.User.LastName));
    }
}